...
# downloader
def downloader(exchange_id: str, symbol: str, start: str, end: str | None = None, timeframe: str = '5m', limit: int = 1000) -> pd.DataFrame:
    ...
    if binance.has["fetchOHLCV"]:
        ...
        for limit in limits:
            if ohlcv:
                ts = dt_from_ts(ohlcv[-1][0]) + timedelta(seconds=timeframe_to_seconds(timeframe))
                since = dt_ts(ts)
