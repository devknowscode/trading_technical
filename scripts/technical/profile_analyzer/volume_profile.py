...
class VolumeProfile(__ProfileAnalyzer):
    ...
    def fit(self):
        ...
        profile = pd.DataFrame({"Price": np.round(bins, 2), "Volume": volume_counts})
    ...
    def poc(self):
        poc_idx = np.argmax(self.profile["Volume"])
    ...
    def value_area(self):
        ...
        va = total_volume * self.perc / 100

        cumulative_volume = 0
        value_prices = []
        profile = self.profile.sort_values(by="Volume", ascending=False)

        for _, row in profile.iterrows():
            value_prices.append(row["Price"])
            cumulative_volume += row["Volume"]
            if cumulative_volume > va:
                break
    ...
    def plot(self):
        ...
        axes[0].axhline(
            self.poc[1], color="#fff", linestyle="-", linewidth=1, label="POC"
        )
        ...
        alphas = np.where(
            (self.profile["Price"] >= self.va[0])
            & (self.profile["Price"] <= self.va[1]),
            1,
            0.5,
        )
        ...
        axes[1].barh(
            ...
            self.profile["Volume"][:-1],
            height=np.diff(self.profile["Price"]),
        ...
        axes[1].text(
            0, self.va[0] * 0.995, "◀ VAL", fontsize=8, fontweight="bold", color="white"
        )
        axes[1].text(
            0, self.va[1] * 0.995, "◀ VAH", fontsize=8, fontweight="bold", color="white"
        )
        ...
        axes[1].tick_params(
            ...
            labelleft=False,
            labelbottom=False,
        )  # remove tick x and y axis on axes[1]
        axes[1].set_title("Volume Profile", fontweight="bold", color="#FFB22C")
        axes[1].invert_xaxis()
