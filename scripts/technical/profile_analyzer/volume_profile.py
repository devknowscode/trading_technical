...
class VolumeProfile(__ProfileAnalyzer):
    ...
    def plot(self):
        ...
        # Plot to chart
        axes[1].barh(self.profile["Price"][:-1], self.profile["Volume"][:-1], height=np.diff(self.profile["Price"]), orientation="horizontal", color=rbga, edgecolor="black")
        ...
        axes[1].tick_params(top=False, bottom=False, left=False, right=False,
                labelleft=False, labelbottom=False)  # remove tick x and y axis on axes[1]
