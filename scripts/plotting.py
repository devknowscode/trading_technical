...
class Plotting():
    def __init__(self, symbol: str, timeframe: str = "5m", limit: int = 1000, interval: int = 1):
        """
        ...
        self.poc_line_0 = self.axes[0].axhline(0, color="white", linestyle="-", linewidth=1.5)
        self.poc_line_1 = self.axes[1].axhline(0, color="white", linestyle="-", linewidth=1.5)
        ...
        self.axes[0].set_title(f"Bitcoin Price Chart - {self.timeframe}", fontweight="bold", color="#FFB22C")
    ...
    def load_data(self, start: str, end: str | None, exchange_id: str):
        ...
        since, until = datetime.fromisoformat(start), datetime.fromisoformat(end) if end else None
    ...
    def plot(self, start: str, end: str | None = None, exchange_id: str = "binance", profile_type: str = "MarketProfile"):
        ...
        # Plot to chart
        self.profile_bars = self.axes[1].barh(profile["Price"][:-1], profile["TPOs"][:-1] if profile_type == "MarketProfile" else profile["Volume"][:-1],
        ...
        self.axes[1].set_title("Market Profile" if profile_type == "MarketProfile" else "Volume Profile", fontweight="bold", color="#FFB22C")
    ...
    def update(self, frame, exchange_id: str, profile_type: str):
        ...
        if self.data_path.exists():
            data_csv = pd.read_csv(self.data_path, index_col=["Date"], parse_dates=["Date"])
            since = data_csv.index[-1].isoformat()
        else:
            since = (datetime.now() - timedelta(seconds=(timeframe_to_seconds(self.timeframe) * 86400))).isoformat()
        
        downloader(exchange_id=exchange_id, symbol=self.symbol, start=since, timeframe=self.timeframe)
    ...
    def live(self, exchange_id: str = "binance", profile_type: str = "MarketProfile"):  
        anim = FuncAnimation(self.fig, self.update, interval=self.interval * 1000, cache_frame_data=False, fargs=(exchange_id, profile_type,))
        plt.show()
